...
const Community = () => {
  ...
  const fetchProjects = async () => {
    setLoading(true);
    try {
      const { projects } = await publicApi.list();
      setProjects(projects);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to load community projects');
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className='flex flex-wrap gap-3.5'>
              {projects.map((project) => (
                <Link
                  ...
                  key={project.id}
                  target='_blank'
                  className='w-72 max-sm:mx-auto  bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden cursor-pointer  group hover:border-indigo-800/80 transition-all duration-300'
                >
